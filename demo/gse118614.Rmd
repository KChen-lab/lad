...
```{r}
DimPlot(obj, reduction = "umap", group.by = c("sample", "short_celltype"), label = T, repel = T, pt.size = 1.)
```
...
```{r}
obj <- RunUMAP(obj, reduction = "alt", dims = 1:10, verbose=FALSE, reduction.name = "altumap")
```
```{r}
DimPlot(obj, reduction = "altumap", group.by = c("sample", "short_celltype"), label = T, repel = T, pt.size = 1.)
```